...
export function Hero() {
  return (
    <section id="top" className="relative isolate flex min-h-svh flex-col overflow-hidden pt-28 sm:pt-32">
      <PaperTooth />

      <div className="relative z-20 mx-auto flex w-full max-w-[1440px] flex-1 flex-col justify-center px-5 pb-[30vh] sm:px-8 sm:pb-[36vh] lg:px-12">
        <Reveal className="flex flex-wrap items-baseline gap-x-5 gap-y-1">
          <span className="sheet-label">L-0.0 — Cover</span>
          <span className="h-px w-8 bg-cypress/25" aria-hidden="true" />
          <span className="sheet-label">{site.region}</span>
          <span className="h-px w-8 bg-cypress/25" aria-hidden="true" />
          <span className="sheet-label">Family owned since {site.founded}</span>
        </Reveal>

        <h1 className="mt-7 sm:mt-10">
          <span className="sr-only">
            Noble Saint builds landscapes, hardscapes and pools in Southern California.
          </span>
          {lines.map((l, i) => (
        ...
        <Reveal delay={560} className="mt-9 flex max-w-[62ch] flex-col gap-7">
          <p className="text-[length:var(--text-sub)] leading-relaxed text-cypress/80">
            A father and his oldest son, building outdoor space across Southern California for twenty
          ...
          <div className="flex flex-wrap items-center gap-3">
            <a
              href="#inquiry"
              className="group flex items-center gap-3 bg-cypress px-6 py-3.5 text-limestone transition-colors hover:bg-patina"
            >
              <span className="text-[0.92rem] tracking-wide">Start a project</span>
            ...
            <a
              ...
              className="border border-cypress/30 px-6 py-3.5 text-[0.92rem] tracking-wide transition-colors hover:border-cypress hover:bg-cypress/5"
            >
              Call {site.contact.person.split(" ")[0]} — <span className="tabular-nums">{site.contact.phone}</span>
            </a>
          </div>
        </Reveal>
      </div>
      ...
      <Reveal
        threshold={0}
        className="pointer-events-none absolute inset-x-0 bottom-0 z-10 h-[32vh] min-h-[210px] w-full sm:h-[46vh] sm:min-h-[300px] text-cypress"
      >
        <SiteSection />
      </Reveal>

      <div className="absolute right-5 bottom-6 z-20 flex items-end gap-5 sm:right-8 lg:right-12">
        <span className="sheet-label hidden sm:block">Section A–A · 1:100</span>
        <NorthArrow className="h-11 w-auto text-cypress/60" />
      </div>
    </section>
  );
}
